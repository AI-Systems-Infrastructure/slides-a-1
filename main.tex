...
\textbf{A unique identifier} pointing to some network resource, with \textit{one or more corresponding IP addresses}.
    ...
    \vspace{0.5em}
    Technically, the network still uses IP addresses under the hood, translating domains on the fly.

    But we as humans use domains to identify APIs in most cases.
\end{frame}
...
\begin{frame}[fragile]{Body}
    The content of the request, used with the \texttt{POST} method that sends data.
...
\begin{frame}{Why the browser is not enough}
    ...
    \begin{itemize}
        ...
        \item Unintuitive way to inspect the raw response components
    \end{itemize}
...
AI APIs almost always \textit{require authentication}.
        \begin{itemize}
            ...
            \item Google is relatively generous, with some lighter models usable free of charge
        \end{itemize}
    \end{block}
\end{frame}
